...
class GpxParserService {
  ...
  static List<GpTrack> _parseTracksFromDocument(XmlDocument document, bool isGpx) {
    ...
    for (final XmlElement note in document.findAllElements('Note')) {
      ...
      bool isGhost = (note.findAllElements('Ghost').firstOrNull?.innerText.toLowerCase() == 'true') || _hasPropertyOrNode(note, ['Ghost']);
      
      HarmonicType harmonicType = HarmonicType.none;
      final harmonicNode = note.findAllElements('Harmonic').firstOrNull;
      if (harmonicNode != null || _hasPropertyOrNode(note, ['Harmonic', 'HarmonicType'])) {
        final typeText = (harmonicNode?.getAttribute('type') ?? harmonicNode?.getAttribute('flags') ?? harmonicNode?.innerText ?? '').toLowerCase();
      ...
      if (slideNode != null || _hasPropertyOrNode(note, ['Slide'])) {
      ...
      // 1. Scan for GP5 standard <Bend><Point/BendPoint>
      XmlElement? bendNode = note.findAllElements('Bend').firstOrNull;
      if (bendNode != null) {
        final pointNodes = bendNode.findAllElements('Point').isNotEmpty ? bendNode.findAllElements('Point') : bendNode.findAllElements('BendPoint');
        for (final pt in pointNodes) {
          ...
          if (posStr != null && offStr != null) {
            rawPoints.add((pos: double.tryParse(posStr.trim()) ?? 0.0, val: double.tryParse(offStr.trim()) ?? 0.0));
          }
        }
      }

      // 2. Scan ALL independent property tags (GP7/GP8 standard)
      if (rawPoints.isEmpty) {
        final Map<String, double> propFloats = {};
        for (final prop in note.findAllElements('Property')) {
          final name = prop.getAttribute('name');
          if (name != null && name.startsWith('Bend')) {
            final flt = prop.findElements('Float').firstOrNull?.innerText.trim();
            if (flt != null) {
              final double? val = double.tryParse(flt);
              if (val != null) propFloats[name] = val;
            }
          }
        }
        if (propFloats.isNotEmpty) {
      ...
      // 3. Fallback for primitive bends (No specific coordinate paths)
      if (rawPoints.isEmpty) {
        for (final prop in note.findAllElements('Property')) {
          final name = prop.getAttribute('name');
          if (name == 'Bend' || name == 'Bended') bendNode = prop;
        }
        if (bendNode != null) {
          String? valAttr = bendNode.getAttribute('Value') ?? bendNode.getAttribute('value') ?? bendNode.getAttribute('Offset') ?? bendNode.getAttribute('offset');
          valAttr ??= bendNode.findElements('Float').firstOrNull?.innerText ?? bendNode.findElements('Int').firstOrNull?.innerText;
          if (valAttr != null) {
            final double v = double.tryParse(valAttr.trim()) ?? 0.0;
            if (v != 0.0) {
              rawPoints.add((pos: 0.0, val: 0.0));
              rawPoints.add((pos: isGpx ? 25.0 : 15.0, val: v)); 
              rawPoints.add((pos: isGpx ? 100.0 : 60.0, val: v)); 
            }
          }
        }
      }
      ...
      if (rawPoints.isNotEmpty) {
        ...
        double actualMaxPos = rawPoints.map((p) => p.pos).fold(0.0, max);
        final double maxPos = actualMaxPos > 60.0 ? 100.0 : 60.0;

        for (final raw in rawPoints) {
          final double normalizedPos = (maxPos > 0) ? (raw.pos / maxPos).clamp(0.0, 1.0) : 0.0; 
          
          double offsetSemitones;
          if (raw.val > 32.0) {
            offsetSemitones = raw.val / 50.0; // GP7 / GP5 scale
          } else if (raw.val > 0.0) {
            offsetSemitones = raw.val / 2.0;  // Old GPX Quarter Tones
          } else {
            offsetSemitones = 0.0;
          }
